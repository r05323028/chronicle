...
const site = process.env.SITE ?? (owner ? `https://${owner}.github.io` : 'https://r05323028.github.io');
const base = process.env.BASE_PATH ?? (isGitHubActions && repository ? `/${repository}` : '/');
    ...
    'i18n:setup'({ injectTranslations }) {
      injectTranslations({
        'zh-TW': {
          ...
          'starlightVersions.search.outdated.slug': '搜尋範圍限於 {{slug}} 版本。',
        ...
        ja: {
          ...
          'starlightVersions.outdated.slug': 'このコンテンツは{{slug}}バージョン向けです。',
          'starlightVersions.search.link.latest':
            '最新の検索結果を見るには<a href={{link}}>最新バージョン</a>へ移動してください。',
          'starlightVersions.search.outdated.label': '検索対象は{{label}}に限定されています。',
          'starlightVersions.search.outdated.slug': '検索対象は{{slug}}バージョンに限定されています。',
    ...
    starlight({
      ...
      sidebar: [
        ...
        },
        {
          ...
          ...translated('部署', 'デプロイ'),
          items: [
            { label: 'Local Linux', ...translated('本機 Linux', 'ローカル Linux'), slug: 'docs/deployment/local' },
          ],
        },
        {
          label: 'Reference',
          ...
          items: [
            { label: 'CLI', ...translated('CLI', 'CLI'), slug: 'docs/reference/cli' },
            { label: 'Terminology', ...translated('術語', '用語'), slug: 'docs/reference/terminology' },
          ],
        },
        {
