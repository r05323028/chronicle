...
Chronicle のリリース版バイナリは Linux の `x86_64` と `aarch64`／`arm64` に対応しています。リポジトリのインストーラーは GitHub Release の設定された安定リリースを取得し、対応するアーカイブを選択し、`SHA256SUMS` を検証してから、検証に成功した場合だけインストールします。
...
- リリースバイナリの target はリトルエンディアンの `x86_64` と `aarch64`／`arm64` です。
