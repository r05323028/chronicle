...
Native causal correlation は opt-in です。Passive-only Chronicle は安全ですが、timing、proximity、process/task/socket/connection identity、wire direction からアプリケーションの因果関係を推測しません。Cooperative Chronicle-native mode は、プロトコルが所有する boundary identity を通じて opaque context と明示的な parent／child handoff を伝達し、ETL は exact bind のみを行い、それ以外は unresolved／ambiguous diagnostic を生成します。tracing provider は不要で、side-channel の損失は fail closed になります。
