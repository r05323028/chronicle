...
最初の安定リリースはまだ公開されていません。現在利用可能なインストール方法は「ソースからビルドする」です。リリース版インストーラーは、最初の公開リリース以降に推奨経路になります。

## リリース版インストーラー（最初の公開リリース以降）

Chronicle のリリース版バイナリは Linux の `x86_64` と `aarch64`／`arm64` に対応する予定です。リポジトリのインストーラーは GitHub Release の設定された安定リリースを取得し、対応するアーカイブを選択し、`SHA256SUMS` を検証してから、検証に成功した場合だけインストールします。
