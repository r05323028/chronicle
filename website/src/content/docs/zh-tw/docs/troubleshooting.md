...
公開 recording 沒有隱含的時間限制；有明確上限的 epoch WAL（預設物理上限 4 GiB）會 rollover 而不會終止 recording。檢查磁碟空間與 recording directory。如果 recording 可以復原，請在不重新擷取的情況下重試 finalization：
