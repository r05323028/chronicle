...
儲存庫也包含適用於支援部署的 continuous recorder。當面向意圖的公開 CLI surface 尚在穩定時，它的 foreground entrypoint 仍保持隱藏。它負責一個 filesystem domain、有明確上限的 epoch rotation、incremental ETL／continuation resume、liveness／health metadata 與 shutdown cleanup；前一個 epoch 的 ETL 落後時，capture 仍可繼續。
