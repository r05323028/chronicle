...
Native causal correlation 是 opt-in 功能。Passive-only Chronicle 仍然安全，但不會從 timing、proximity、process/task/socket/connection identity 或 wire direction 推斷應用程式因果關係。Cooperative Chronicle-native mode 透過協定擁有的 boundary identity 傳遞不透明 context 與明確的 parent／child handoff；ETL 只會 exact bind，否則產生 unresolved／ambiguous diagnostic。不需要 tracing provider；side-channel 遺失時會 fail closed。
