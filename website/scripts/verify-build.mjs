...
const marketingCss = readFileSync(
  path.join(websiteDir, "src", "styles", "marketing.css"),
  "utf8",
);
const starlightCss = readFileSync(
  path.join(websiteDir, "src", "styles", "starlight.css"),
  "utf8",
);
const expressiveCodeConfig = readFileSync(
  path.join(websiteDir, "ec.config.mjs"),
  "utf8",
);
...
if (marketingCss.includes("width: min(100%, var(--content-width));")) {
  errors.push(
    "marketing surfaces still use a capped width instead of full-bleed layout.",
  );
}
if (!marketingCss.includes("--page-gutter:"))
  errors.push("marketing page gutter token missing.");
for (const token of ["--code-bg:", "--code-fg:", "--code-red:"]) {
  if (!marketingCss.includes(token))
    errors.push(`marketing code token missing: ${token}`);
  if (!starlightCss.includes(token))
    errors.push(`Starlight code token missing: ${token}`);
}
...
for (const token of [
  ...
  if (!normalizedEcConfig.includes(token))
    errors.push(`Expressive Code padding override missing: ${token}`);
}
for (const token of ["--color-paper: #f2efe7;", "--color-night: #121918;"]) {
  if (!marketingCss.includes(token))
    errors.push(`marketing page palette missing: ${token}`);
}
for (const token of [
  "--chronicle-paper: #f2efe7;",
  "--chronicle-night: #121918;",
]) {
  if (!starlightCss.includes(token))
    errors.push(`Starlight page palette missing: ${token}`);
}
for (const token of [
  "tokyo-night",
  "tokyo-night-day",
  ...
  if (!expressiveCodeConfig.includes(token))
    errors.push(`Expressive Code Tokyo Night token missing: ${token}`);
}
...
function routeForLink(raw, source) {
  ...
  if (withoutFragment.startsWith("/")) {
    if (
      base !== "/" &&
      withoutFragment !== base &&
      !withoutFragment.startsWith(`${base}/`)
    ) {
    ...
    return base === "/"
      ? withoutFragment
      : withoutFragment.slice(base.length) || "/";
  }

  const sourceDirectory = path.posix.dirname(`/${source}`);
  return path.posix.normalize(
    path.posix.join(sourceDirectory, withoutFragment),
  );
}
...
for (const source of htmlFiles) {
  ...
  for (const match of html.matchAll(/(?:href|src)=["']([^"']+)["']/g)) {
    ...
    if (route && !routeFile(route))
      errors.push(`missing asset or route: ${source} -> ${match[1]}`);
  }
}
...
const versionSlugs = readdirSync(path.join(docsDir), { withFileTypes: true })
  .filter(
    (entry) =>
      entry.isDirectory() && !["docs", ...locales].includes(entry.name),
  )
...
const requiredRoutes = [
  ...
  ...locales.map((locale) => `/${locale}/docs/`),
];
...
if (!existsSync(pagefindDir) || !readdirSync(pagefindDir).length)
  errors.push("Pagefind output missing.");
...
if (latestDocs && firstVersion) {
  ...
  if (!latestHtml.includes(`${pathPrefix}/${firstVersion}/`))
    errors.push("Latest docs has no archived-version switch link.");
  if (!archivedHtml.includes(`${pathPrefix}/docs/`))
    errors.push("Archived docs has no Latest switch link.");
  if (!latestHtml.includes("pagefind"))
    errors.push("Latest docs has no Pagefind search asset.");
...
for (const locale of ["", ...locales]) {
  ...
  for (const other of ["", ...locales].filter(
    (candidate) => candidate !== locale,
  )) {
    const target = other ? `${pathPrefix}/${other}/` : `${pathPrefix}/`;
    if (!html.includes(target))
      errors.push(`locale switch missing: ${route} -> ${target}`);
  }
}

if (errors.length > 0) {
  fail(
    `Static output verification failed (${errors.length} issue${errors.length === 1 ? "" : "s"}):`,
  );
  fail(
    errors
      .slice(0, 20)
...
log(
  `Verified ${htmlFiles.length} HTML pages, ${versionSlugs.length} archived version${versionSlugs.length === 1 ? "" : "s"}, locales en/zh-tw/ja, base ${base}, internal routes/assets, locale/version switches, and Pagefind.`,
);
