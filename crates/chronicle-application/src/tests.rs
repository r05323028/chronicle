...
fn ingest_discards_only_wal_limit_suffix_and_wal_limit_wins_duration_tie() {
    ...
    let frame_bytes = (chronicle_wal::MIN_SEGMENT_BYTES
        ...
        + 1;
    let payload_bytes =
        usize::try_from(frame_bytes - u64::try_from(chronicle_wal::ENVELOPE_HEADER_LEN).unwrap())
            .unwrap();
    assert_eq!(
...
fn replay_context_loads_only_redacted_runtime_authorization() {
    ...
    let context = replay_context_from(&config, |_| Some("Bearer runtime-secret".into())).unwrap();
    assert_eq!(
...
fn record_fixture_does_not_publish_when_wal_preflight_fails() {
    let root = std::env::temp_dir().join(format!("chronicle-record-fail-{}", uuid::Uuid::new_v4()));
    let fixture = std::fs::read(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/http/basic-session.json"
    ))
...
fn capture_source_is_written_and_read_back_through_wal() {
    ...
    let recorded = write_capture_to_wal(&mut source, &directory, RecordingId::new(), 2048).unwrap();
...
fn repeated_identical_capture_is_deduplicated_in_final_session() {
    ...
    let root = std::env::temp_dir().join(format!("chronicle-app-dedup-{}", uuid::Uuid::new_v4()));
...
fn corrupt_wal_prevents_processing_before_session_publication() {
    ...
    let root = std::env::temp_dir().join(format!("chronicle-app-corrupt-{}", uuid::Uuid::new_v4()));
    let wal_directory = root.join("wal");
    let recorded = write_capture_to_wal(
        &mut InMemoryCaptureSource::new(test_capture_events(b"GET / HTTP/1.1\r\n\r\n".to_vec())),
        &wal_directory,
        RecordingId::new(),
        2048,
    )
...
fn oversized_capture_is_rejected_before_wal_directory_creation() {
    ...
    let error = write_capture_to_wal(&mut source, &directory, RecordingId::new(), 64).unwrap_err();
...
fn wal_preflight_is_exclusive_and_reports_free_space_without_metadata() {
    let root = std::env::temp_dir().join(format!("chronicle-preflight-{}", uuid::Uuid::new_v4()));
    std::fs::create_dir_all(&root).unwrap();
    let wal = root.join("wal");
    let preflight = preflight_wal_destination(&wal, ProductionRecordingBounds::default()).unwrap();
...
fn recovery_report_is_body_safe_versioned_and_private() {
    ...
    let directory = std::env::temp_dir().join(format!("chronicle-report-{}", uuid::Uuid::new_v4()));
...
fn recovery_transaction_reports_before_discard_and_hands_off_lock() {
    use chronicle_wal::{MIN_SEGMENT_BYTES, WalRecordEnvelope, encode_envelope, segment_file_name};
    ...
    let persisted =
        decode_recovery_report(&fs::read(wal_directory.join("etl/recovery-report.json")).unwrap())
            .unwrap();
