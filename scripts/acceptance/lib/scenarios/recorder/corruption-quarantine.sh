...
PY
	...
	set +e
	"$CHRONICLE" --format json internal etl --wal-dir "$CORRUPT_WAL" --output "$CORRUPT_STORE" >"$ARTIFACT_ROOT/corruption.json" 2>"$ARTIFACT_ROOT/corruption.log"
	corruption_status=$?
	"$CHRONICLE" --format json internal etl --wal-dir "$CORRUPT_WAL" --output "$CORRUPT_STORE-retry" >"$ARTIFACT_ROOT/corruption-retry.json" 2>"$ARTIFACT_ROOT/corruption-retry.log"
