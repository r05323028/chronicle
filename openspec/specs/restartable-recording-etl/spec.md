...
`FinalSessionCheckpoint v1` SHALL remain the existing stopped-recording final-session artifact. `IncrementalEtlCheckpoint` SHALL be the single current incremental ETL cursor/decoder authority with an explicit persisted schema version; it SHALL bind typed parent `RecordingId`, current `EpochId`/ordinal, optional unique predecessor/successor IDs, exact marker/segment lineage, continuation-in/out references and digests, decoder/schema/pipeline versions, bounded decoder state, emitted operation fingerprints, immutable output identities, source status, and checksum. `EpochContinuationCheckpoint` SHALL be a separate bounded handoff artifact whose identity binds parent, predecessor, successor, marker/segment lineage, decoder/schema/pipeline versions, and checksum.

V2 restore SHALL reject unknown versions, v1/v2 discriminator confusion, parent/epoch/predecessor/successor mismatch, marker/cursor mismatch, lineage fork/gap, checksum failure, unsupported decoder kind/version, duplicate/inconsistent state, and bound/configuration overflow before output or retention eligibility. A fresh decoder SHALL never replace failed continuation restore.
