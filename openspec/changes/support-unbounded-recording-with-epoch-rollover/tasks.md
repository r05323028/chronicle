...
- [ ] 3.2 Implement parent-aware `RolloverTransitionV2` phases for successor allocation, predecessor final marker, outcome journal, continuation checkpoint, catalog activation, and quota reservation.
...
- [ ] 3.4 Keep capture source/queue alive across rollover, account admitted observations, and stop visibly on reservation, handoff, continuation, or activation failure.
- [ ] 3.5 Add rollover crash/fault tests for every phase, conflicting summaries/catalog/journal/WAL evidence, quota pressure, retention, and no duplicate successor.
...
- [ ] 4.2 Export continuation from predecessor only after its authoritative marker and seed successor only from the unique verified predecessor; make handoff/recovery idempotent.
...
- [ ] 4.5 Add checkpoint round-trip, checksum/version/identity/lineage mismatch, crash handoff, state exhaustion, and multi-rollover boundedness tests.

## 5. Incremental ETL and deterministic publication

- [ ] 5.1 Keep per-epoch snapshot/publication/checkpoint/retention boundaries while allowing `IncrementalProcessor` to restore one verified predecessor continuation seed.
...
- [ ] 5.5 Add ETL tests for HTTP request/response rollover, long-lived TCP, pipelining/correlation, crash handoff, invalid continuation, bound exhaustion, multiple rollovers, lag, and fork/gap recovery.
...
- [ ] 6.4 Implement independent source-WAL/derived-artifact retention, continuation-reference retention, digest-preconditioned deletion, tombstones, and orphan handling.
...
- [ ] 8.2 Expose per-epoch continuation state, checkpoint v1/v2 status, lag, quota, retention, rollover, and safe remediation without payload values.
...
- [ ] 9.4 Add privileged acceptance scenarios for live multi-epoch capture, concurrent ETL, continuation handoff/crash/recovery, bounded state, replay, quota, retention, and cleanup evidence.
