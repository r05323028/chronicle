...
B1.1 establishes discriminator-first compatibility scaffolding without freezing future DTOs. Current writers remain WAL v1, canonical v2, and manifest v1. WAL v2, canonical v3, and manifest v2 identifiers are reserved, but current readers return explicit unsupported-version errors until B2/B3, C3, and D1 activate their respective DTO decoders. Version zero, gaps, and versions newer than the reserved set are rejected. B1.5 now freezes only kernel-independent `recording.json` core fields needed by recovery: recording ID, optional immutable selector path/cgroup ID, lifecycle status, separate shutdown reason, last-valid-marker boundary, and stable categorized counters. Capture-specific build/kernel/capability/scope fields remain B7.3 after feasibility. This scaffolding spans `chronicle-common`, `chronicle-wal`, `chronicle-canonical`, `chronicle-storage`, and `chronicle-application`; it MUST NOT serialize an older shape under a reserved future version.
