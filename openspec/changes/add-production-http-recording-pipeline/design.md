...
Capture Event v2 is capture-domain data only. It represents kernel monotonic timestamp and optional wall-clock correlation; connect-attributed PID/TGID and stable cgroup identity joined to later evidence by socket cookie; candidate boot/socket/generation identity plus tuple; raw lifecycle evidence (`established`, `state_changed`, `closed`, `reset`, or `unknown`); direction/TCP position/payload; and observed length/truncation/flags. Reset/closed values require an actually proven signal; sock-ops state alone remains `state_changed` or `unknown`. `LossWindow` is a separate typed persistence payload derived from capture counters; exact kernel-derived fields remain feasibility-gated.

`WalRecordEnvelope` owns persistence identity and order: recording ID, WAL sequence, segment ordinal/first sequence, byte offset, `RecordKind`/schema version, and encoded payload. `CaptureEvent`, `LossWindow`, and `CommitMarker` record kinds all consume the same monotonically increasing envelope sequence. Segment/offset become known while writing/recovery, not while capturing. B2.2 therefore defines one envelope with explicit unplaced (`provenance = None`) and placed states; persistence assigns sequence while wrapping an opaque payload, and writer/recovery attaches provenance. B3.2 alone freezes binary framing, payload length, CRC32C, and version-dispatch bytes, and requires placed provenance when yielding decoded records. Fixture and in-memory Capture Event v2 values therefore require no WAL sequence. Existing fixture v1 `sequence` remains readable as fixture-source ordering and is normalized without changing P0 bytes; persisted ordering still comes from its WAL v1 record. File descriptor is provenance only. Synthetic connection identity may use recording ID plus first enclosing WAL sequence after persistence and is always marked incomplete.

B1.1 establishes scoped version validation without freezing future DTOs. WAL and manifest retain their independent current/future version contracts. Canonical has one evolving MVP schema: writer and reader use only `CANONICAL_SCHEMA_VERSION = 1`; non-v1 values fail explicitly. Canonical has no v2/v3 identifiers, compatibility dispatcher, migration defaults, or nested version namespace because no stable canonical artifact has shipped. New optional MVP metadata is append-only under v1. A future canonical v2 is reserved only for a stable-release breaking semantic, required-field, payload-meaning, or replay-behavior change. B1.5 now freezes only kernel-independent `recording.json` core fields needed by recovery: recording ID, optional immutable selector path/cgroup ID, lifecycle status, separate shutdown reason, last-valid-marker boundary, and stable categorized counters. Capture-specific build/kernel/capability/scope fields remain B7.3 after feasibility. This scaffolding spans `chronicle-common`, `chronicle-wal`, `chronicle-canonical`, `chronicle-storage`, and `chronicle-application`.

### 4a. Normalize capture evidence before reconstruction

`ReconstructionInput` is a small reconstruction-domain contract between capture compatibility adapters and later stream reconstruction. Fixture v1 and production Capture Event v2 both adapt into it; neither is converted into the other's model. It carries fixture or socket identity, monotonic timestamp with an explicit fixture-derived marker, direction, TCP sequence/continuation when observed, raw payload/truncation, raw lifecycle evidence, recording scope identity, and separate loss-window evidence where applicable. It contains no WAL sequence/segment/offset, Aya or kernel ABI value, HTTP field, reconstructed message, or fixture-only persistence field.

This contract intentionally does not order, deduplicate, reassemble TCP, derive termination or loss ambiguity policy, apply timeouts, or parse HTTP. Those behaviors remain C1/C2 work. B2.3 adapts existing fixture v1 ordering through this contract before B2.4 adds deterministic v2 fixture/loss observations, resolving the prior dependency inversion without freezing later reconstruction behavior.
...
Each v2 segment starts with an exact 64-byte little-endian header. Bytes 0..4 are `CHS2`; 4..6 are format version 2; 6..8 are header schema version 1; 8..12 are header length 64; 12..28 are raw recording UUID bytes; 28..36 are segment ordinal; 36..44 are first WAL sequence; 44..52 are signed Unix-nanosecond creation time; 52..60 are zero reserved bytes; and 60..64 are CRC32C over bytes 0..60. Readers reject wrong magic, format/header version, length, reserved bytes, checksum, recording identity, ordinal, first sequence, or truncated header. Each v2 `WalRecordEnvelope` frame has an exact 48-byte little-endian header. Bytes 0..4 are `CHE2`; 4..6 are envelope version 1; 6..8 are `RecordKind`; 8..10 are flags; 10..12 are payload schema version; 12..16 are header length 48; 16..20 are payload length; 20..36 are raw recording UUID bytes; 36..44 are WAL sequence; and 44..48 are CRC32C over bytes 0..44 followed by payload. Payload bytes immediately follow the header. Segment ordinal, segment first sequence, and frame byte offset remain envelope-owned in-memory provenance derived from the validated segment header and physical reader/writer position; they are not duplicated in each frame. Readers retain explicit v1 support for P0 fixture artifacts and reject unknown magic, envelope version, kind, length, identity, or sequence before payload allocation.

Published v2 segments use exact 20-digit `<first-sequence>.chwal` names. Header-only discovery ignores dot-prefixed temporary files, sorts by numeric filename, validates recording identity and filename/header first-sequence equality, requires unique contiguous 0-based ordinals and strictly increasing first sequences, and rejects other entries. It does not infer frame ranges from headers; exact missing/overlapping envelope sequence validation belongs to B5.1 commit-aware recovery scan.
...
Rollback disables/removes live `record` exposure while preserving WAL v2 readers and canonical MVP-v1 readers. Existing P0 fixture record/inspect/replay remains usable. Persisted newer artifacts are never rewritten down-level; older binaries reject unknown versions clearly.

## Open Questions

No user decision blocks proposal, but capture-specific implementation is gated. Gate A MUST confirm exact Aya APIs/helper availability, socket identity propagation, raw lifecycle state evidence (including whether directional half-close can be derived), direction/tuple normalization, GSO/GRO/nonlinear-skb handling, stable per-CPU counter/map identity and 100 ms/final-sample behavior, and cgroup subtree/process-enumeration behavior. Failure requires updating/revalidating this change before freezing Capture Event v2/eBPF ABI/support matrix. Generic WAL, recovery, metadata, JSON, replay-result, and filesystem primitives MAY continue independently but MUST NOT be cited as live-capture proof.
