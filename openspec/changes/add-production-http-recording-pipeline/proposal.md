...
- Replace the one-segment, buffer-all fixture recording path for live capture with bounded capture-to-WAL flow, segmented WAL v2 using one bounded group-commit policy, explicit received/queued/written/durable/loss counters, recording metadata, crash recovery, and safe reopen.
...
- Add standalone, restartable, idempotent ETL over recovered WAL segments, including connection-generation identity, TCP ordering/deduplication, typed loss-window attribution, provenance, deterministic session identity, and atomic no-replace publication/checkpoint ordering.
...
- `recoverable-recording-wal`: Durable segmented capture WAL, framing/versioning, bounded group commit, rotation/total-size limits, recovery, corruption handling, backpressure, durable-watermark semantics, and persistence counters.
...
- **Formats/APIs:** New capture event and WAL segment/envelope versions, recording status plus shutdown reason, durable watermark/loss-window metadata, canonical completeness/provenance fields, replay outcome, and versioned JSON summaries require backward-compatible readers or explicit rejection of unknown newer versions. Existing P0 fixture/canonical/WAL artifacts remain readable.
