...
- [ ] F2.8 `[automated recovery tests, privileged acceptance]` Use automated fault matrices and fake-clock tests for platform-independent marker/sync/corruption and delayed-poll semantics. Privileged evidence verifies real eBPF capture, periodic/final sampling, abrupt recorder termination recovery, and cleanup on supported Linux; never relabel Cargo tests as privileged evidence; depends on F2.7.
- [ ] F2.9 `[automated WAL-limit tests, privileged acceptance]` Use automated tests for deterministic cap/reservation/discard/final-marker failure semantics. Privileged evidence drives real captured traffic under configured physical bounds and verifies persisted counters/status, ETL rerun/second root, and cleanup; depends on F2.8.
- [ ] F2.10 `[automated replay tests, privileged acceptance]` Use automated tests for complete result accounting and transport-stop semantics. Privileged evidence replays real captured operations only to explicit local target and proves original destination receives no replay traffic; depends on F2.9.
- [ ] F2.11 `[automated selector/signal tests, privileged acceptance]` Use automated matrices for deterministic descendant/race/forbidden-scope and signal failure branches. Privileged evidence verifies real dedicated/shared cgroup selection, multithreaded TGID behavior, SIGINT/SIGTERM/bounds, supported-host attach/detach, and leak-free process/cgroup/eBPF cleanup; depends on F2.8-F2.9.
...
- [ ] F3.6 `[Linux eBPF profile, evidence/privileged/]` Run and retain privileged acceptance for exact clean commit with kernel/arch/capabilities and per-scenario results. Reports under ignored `target/` are transient, not retained evidence; depends on F2.7-F2.12, F3.5.
- [ ] F3.7 `[openspec/changes/add-production-http-recording-pipeline]` Run strict OpenSpec validation, stale-language audit, task/spec/implementation comparison, and independent recovery/replay/security review before archive. OpenSpec validation is specification evidence only and must not be used as runtime completion evidence; depends on F3.5-F3.6.

### F4. Verification remediation

- [x] F4.1 `[chronicle-canonical, chronicle-session, chronicle-protocol, chronicle-protocol-builtins, chronicle-etl, chronicle-replay]` Implement authoritative typed operation completeness and exact contributing WAL byte-range/connection-generation provenance through reconstruction, HTTP canonicalization, storage, inspect, and replay; cover split headers, missing provenance, malformed/unsupported/truncated/unmatched states, and mixed-session replay.
- [x] F4.2 `[chronicle-protocol-builtins, chronicle-etl, chronicle-application, chronicle-cli tests]` Add focused coverage for HTTP body/framing/extension-method edges, ETL idle/batch/publication/checkpoint conflicts, doctor warning/failure/no-side-effect behavior, and atomic JSON serialization/render failure boundaries.
- [x] F4.3 `[scripts/acceptance, scripts/tests/acceptance, docs, evidence/privileged]` Bind full acceptance to expected clean commit, separate automated from privileged results, require explicit environment-specific scenario results for retained completion, use portable report paths, and add host-to-Multipass retained-evidence workflow.
- [ ] F4.4 `[workspace, chronicle-ubuntu]` Pass fmt/check/Clippy/workspace tests, strict OpenSpec validation, isolated eBPF compile, and full retained privileged acceptance on Ubuntu 24.04 for exact commit; retain machine-readable evidence outside ignored build output.
