...
- [ ] F3.6 `[Linux eBPF profile]` Run/retain privileged acceptance with kernel/arch/capabilities/results; do not claim runtime coverage when skipped; depends on F2.7-F2.12, F3.5. **Blocked: current host is not supported Linux; no retained P1 privileged report.**
- [ ] F3.7 `[openspec/changes/add-production-http-recording-pipeline]` Run strict validation, stale-language audit, task/spec/implementation comparison, and independent recovery/replay/security review before archive; depends on F3.5-F3.6. **Blocked on F3.6 privileged evidence.**
