...
- [x] D2 `[chronicle-storage]` Preserve private staging/sync/manifest-last/no-replace publication for v3 and session-qualified SHA-256 payload refs; rerun permission/concurrency/failure tests; depends on D1.
...
- [x] F3.7 `[openspec/changes/add-production-http-recording-pipeline]` Run strict validation, stale-language audit, task/spec/implementation comparison, and independent recovery/replay/security review before archive; depends on F3.5-F3.6.
