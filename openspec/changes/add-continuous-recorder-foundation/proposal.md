...
- Add bounded lifecycle-index compaction for epoch lineage, tombstones, and retention metadata, with crash recovery and contradiction detection. Define accepted observation at recorder-owned queue admission with stable ingest identity and durable rollover outcome ranges for exact loss accounting. Add explicit implementation gates for lifecycle, incremental ETL feasibility, publication correctness, and retention/quota safety. Add rootless crash/fault/conformance tests and separate privileged Ubuntu 24.04 evidence for continuous eBPF capture, rotation, restart, cleanup, incremental ETL, signal handling, and leak-free process/cgroup/eBPF teardown.
