...
Correlation semantics stay in `chronicle-canonical`; neutral IDs stay in `chronicle-common`. No standalone correlation crate is justified. The repository's existing architecture validation mechanism checks Chronicle workspace dependency direction, critical forbids, semantic/API boundaries, and two external-package guards driven by `[external_dependencies]` in `validation/architecture.toml`: (1) core/domain leakage — `chronicle-common` and `chronicle-canonical` declare no forbidden provider package in any dependency kind; (2) default-distribution closure leakage — no crate reachable from `default_distribution_roots` (`chronicle-cli`, the public `chronicle` executable) through transitive normal/build workspace edges declares one, including renamed, optional, target-specific, and feature-gated declarations; dev-only declarations never link into the executable and stay outside guard (2). dev-only declarations never link into the executable and stay outside guard (2). Guard (2) is enforced twice and independently: at declaration level over the workspace-edge closure — intentionally conservative, rejecting even currently unresolved provider declarations — and at release-graph level by one locked, package-selected `cargo tree --locked -p <root> --target <t> --edges normal,build` query per root and supported release target (`default_distribution_targets`, mirroring the release workflow matrix), so a provider arriving transitively through a third-party crate is rejected while feature activation by separately distributed adapters outside the closure cannot fabricate a false positive. Tree edges carry real package names, so renames never evade the check and multiple versions of one package each count. Diagnostics print the target plus dependency path from the distribution root. The denylist uses audited crates.io package identities (OpenTelemetry API/SDK/exporter bridges, Datadog integrations, AWS X-Ray, Jaeger/Zipkin exporters/propagators); generic `tracing` facades remain allowed and lightweight trace-context wire-format parsing is not treated as a provider SDK. Do not create a parallel validator. Separately distributed adapter/plugin crates outside the default closure may depend on provider packages and translate SDK data into Chronicle-owned `CorrelationEvidence`; provider installation UX and runtime plugin discovery remain future changes.
