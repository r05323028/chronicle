...
- [ ] 2.2 Define separate `InteractionRoleResolution` state with `Known(Ingress)`, `Known(Egress)`, `Unknown`, and `Ambiguous` candidate/evidence forms; do not add uncertainty variants to `InteractionRole` itself.
...
- [ ] 2.4 Preserve missing ownership as `Unknown` and conflicting ownership as `Ambiguous`; retain evidence and never choose a default through timing, PID, connection ownership, socket role, or ordering.
...
- [ ] 5.5 Test conflicting ingress/egress evidence -> `Ambiguous` candidate roles with preserved evidence and root rejection.
...
- [ ] 5.7 Test role/correlation independence with `Known(Ingress)+Resolved`, `Known(Egress)+Ambiguous`, `Unknown+Uncorrelated`, and `Ambiguous+Ambiguous` supplied outcomes.
...
- [ ] 5.11 Test ambiguous A/B candidates retain candidate-specific evidence and remain outside memberships/edges; test uncorrelated operations remain in the aggregate without synthetic owners.
