...
- Add an always-on recorder service that owns one configured cgroup scope and filesystem domain, persists lifecycle state, recovers before capture readiness, rolls bounded recording epochs without stopping capture, handles SIGTERM/SIGINT idempotently, and resumes after process or host restart. P2 supports one active Chronicle mutating owner per filesystem domain; standalone mutators reject live ownership and read-only commands may continue.
...
- Extend WAL operation from one terminal 4 GiB recording into successive bounded epochs with size- and age-based segment rotation, a crash-safe manifest, sealed-segment integrity digests, configurable retention, synchronized per-filesystem quota/minimum-free-space protection, and checkpoint-plus-final-session-gated cleanup. Existing in-WAL commit markers remain sole authority for committed record bytes. P2 supports one Chronicle mutating writer owner per filesystem domain; multiple recorder instances sharing a WAL/store filesystem are unsupported.
- Add incremental ETL over recovery-authoritative committed snapshots while recording continues. Define separate `FinalSessionCheckpoint v1` for stopped-recording P1 behavior and `IncrementalEtlCheckpoint v1` for P2 live state; persist bounded reconstruction/decoder state and exact input lineage, publish deterministic immutable outputs before advancing the incremental checkpoint, and make publication/checkpoint crash windows idempotent.
- Introduce a `RecordingStore` boundary for immutable recorder artifacts. It owns artifact identity, integrity, lifecycle metadata, immutable persistence, and the filesystem backend only; WAL commit/repair, incremental checkpoint advancement, and final Canonical Session publication remain owned by WAL, incremental ETL, and `FilesystemSessionStore`. P2 ships only a filesystem backend and keeps active WAL on required local durable storage; sealed WAL copies are optional future-compatible artifacts and are not required for retention correctness. Future S3-compatible upload begins only at sealed immutable artifacts and is not implemented here.
...
- Extend diagnostics and CLI/application contracts for daemon preflight, liveness, separate capture/processing readiness, policy-derived health, current epoch/segment, committed boundary, ETL lag, retained bytes, quota headroom, loss counters, recovery state, and safe machine-readable lifecycle control.
- Add bounded lifecycle-index compaction for epoch lineage, tombstones, and retention metadata. Finalized policy-retained, cleanup-complete epochs compact into deterministic lineage anchors preserving first/last identities, predecessor and digest-chain proof, cleanup summary, and contradiction detection; startup/readiness/status never scan unbounded history. Define accepted observation at recorder-owned queue admission with stable ingest identity and durable rollover outcome ranges for exact loss accounting. Add explicit implementation gates for lifecycle, incremental ETL feasibility, publication correctness, and retention/quota safety. Add rootless crash/fault/conformance tests and separate privileged Ubuntu 24.04 evidence for continuous eBPF capture, rotation, restart, cleanup, incremental ETL, signal handling, and leak-free process/cgroup/eBPF teardown.
...
- Existing Canonical Session v1 and deterministic identity rules remain authoritative. `FinalSessionCheckpoint v1` remains P1 final-session state; `IncrementalEtlCheckpoint v1` owns P2 live lineage/state. Incremental outputs use existing canonical values plus provenance wrappers rather than redefining HTTP semantics.
...
- **Artifacts:** new recorder state, WAL manifest, `FinalSessionCheckpoint v1`, `IncrementalEtlCheckpoint v1`, incremental publication index, bounded retention tombstones/epoch lineage indexes, and versioned health/status reports. All are private, checksummed or atomically persisted as appropriate, bounded, and payload-safe by default.
- **Operations:** supported production placement becomes one systemd-managed recorder instance per configured cgroup scope/state root and filesystem domain; dedicated service identity, deterministic domain lease, and local durable filesystem are required.
