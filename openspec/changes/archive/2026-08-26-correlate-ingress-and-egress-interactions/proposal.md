...
- `correlation-domain-model`: adds one additive, non-frozen correlation-evidence variant — resolver-generated `ScenarioRoot { root: CanonicalOperationRef }` — establishing that Chronicle explicitly recorded a `Known(Ingress)` operation as the root of its derived scenario. It is non-temporal correlation evidence; it classifies no interaction role and never replaces `InteractionRoleResolution`. Caller-supplied `ScenarioRoot` remains unauthorized as resolver input (input reservation is owned by the `correlation-resolver` capability). No frozen contract is touched.
