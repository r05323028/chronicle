...
- [x] F2.9 `[automated WAL-limit tests, privileged acceptance]` Use automated tests for deterministic cap/reservation/discard/final-marker failure semantics. Privileged evidence drives real captured traffic under configured physical bounds and verifies persisted counters/status, ETL rerun/second root, and cleanup; depends on F2.8.
- [x] F2.10 `[automated replay tests, privileged acceptance]` Use automated tests for complete result accounting and transport-stop semantics. Privileged evidence replays real captured operations only to explicit local target and proves original destination receives no replay traffic; depends on F2.9.
- [x] F2.11 `[automated selector/signal tests, privileged acceptance]` Use automated matrices for deterministic descendant/race/forbidden-scope and signal failure branches. Privileged evidence verifies real dedicated/shared cgroup selection, multithreaded TGID behavior, SIGINT/SIGTERM/bounds, supported-host attach/detach, and leak-free process/cgroup/eBPF cleanup; depends on F2.8-F2.9.
...
- [x] F3.6 `[Linux eBPF profile, evidence/privileged/]` Run and retain privileged acceptance for exact clean commit with kernel/arch/capabilities and per-scenario results. Reports under ignored `target/` are transient, not retained evidence; depends on F2.7-F2.12, F3.5.
...
- [x] F4.4 `[workspace, chronicle-ubuntu]` Pass fmt/check/Clippy/workspace tests, strict OpenSpec validation, isolated eBPF compile, and full retained privileged acceptance on Ubuntu 24.04 for exact commit; retain machine-readable evidence outside ignored build output.
