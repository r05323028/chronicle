...
- [x] 6.11 Enforce guard (2) with two complementary mechanisms inside the existing validator, both proven by fixture tests (external wrapper introducing a provider rejected; benign external chains pass; dev-only external provider chains stay allowed; plugin-only feature activation outside the closure does not contaminate the default proof while the same feature enabled on the default path is rejected; duplicate-name versions both caught):
  - declaration level: workspace metadata (`cargo metadata --no-deps`) drives the transitive normal/build workspace-edge closure over `default_distribution_roots`, rejecting renamed, optional, target-specific, or feature-gated provider declarations even when currently unresolved;
  - release-graph level: per root and per `default_distribution_target`, one bounded locked package-selected query (`cargo tree --locked --edges normal,build --target <t>`, mirroring the release matrix) proves what the actual release command compiles, so providers entering transitively through third-party crates — including their build dependencies and tree section labels such as `[build-dependencies]` — are rejected while unrelated workspace feature activation cannot fabricate a false positive; `--locked` makes validation fail rather than re-resolve Cargo.lock.
- [x] 6.12 Add standard-library fixture tests for both layers: core/domain rejection, direct and deep transitive closure rejection, rename/optional/target detection inside the closure, dev-dependency exclusion, a separately distributed adapter crate outside the closure depending on `opentelemetry_sdk` being accepted, a missing/empty roots declaration being reported, section-label parsing preserving real parents, and an external transitive build-dependency provider chain being rejected.
...
- [x] 7.4 Run `graphify update .` after production source changes; this change's production surface (canonical correlation domain plus validation tooling) is covered by the fast and act CI-parity gates.
