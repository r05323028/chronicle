...
NativeOperationBoundaryReceipt {
    ...
    protocol_operation_boundary: ProtocolOperationBoundaryClaim,
...
The receipt is therefore an observation of the canonicalizer's boundary, not a second segmentation algorithm. Runtime receipts carry only an opaque `ProtocolOperationBoundaryClaim`; they cannot construct or deserialize the trusted `ProtocolOperationBoundaryIdentity`. A registered `ProtocolCanonicalizer` is the sole source used by `ProtocolRegistry` to establish that identity for the ETL boundary index. The first HTTP adapter accepts `HttpRequestBoundaryObservation`, produced from complete HTTP decoder output with exact source provenance, rather than a bare request sequence. It emits the opaque claim only after those protocol facts are available; the carrier can still hold the handoff while either endpoint receipt is pending. Repeated or pipelined operations on one connection remain distinguishable only through authoritative protocol boundary identity plus generation-safe source and exact provenance; runtime-local ordering cannot repair ambiguity.

The timing boundary is explicit: runtime handoff occurs before canonicalization; the complete `NativeOperationBoundaryReceipt` may become available only after capture, reconstruction, and WAL placement facts are known, but it is still produced before any `CanonicalOperationRef` is required.

Names are semantic contract names, not required final Rust spelling. `NativeOperationBoundaryReceipt` is the minimal new non-frozen fact current code lacks. Its fields are deliberately composed from real current facts: recording/epoch placement from recorder/ETL lineage, `SourceConnectionGeneration`, the decoder-derived protocol-local operation boundary, and exact `WalByteRange` source provenance. A receipt with only PID, TID, task ID, worker name, file descriptor, socket cookie, connection ID, stream ID, WAL sequence, timestamp, or position without its generation-safe scope is invalid.
...
Completion notifications MUST be deterministic and idempotent. Duplicate receipt or handoff delivery produces one complete observation. The carrier stages complete observations until the application-owned channel admits the entire batch; only then does it retire pending state and place a bounded completion tombstone. A failed batch admission enqueues nothing, retains every observation for deterministic retry, and emits typed `SideChannelLoss` diagnostics. If either endpoint never completes, pending state is evicted by the bounded source lifecycle and emits a typed unresolved or bounded-loss diagnostic with no positive native relation. Restart drains queued observations and clears pending/context/tombstone state exactly once: each retained incomplete handoff reports deterministic `RestartLostPending`, queued complete observations report one `SideChannelLoss`, and a second restart with no new state reports nothing. Live contexts, parent receipts, pending entries, tombstones, and diagnostics are bounded; completed child contexts are retired automatically when idle, while roots have explicit retirement. Overflow never silently retains an arbitrary positive subset.
...
- completed handoffs MAY retain only a bounded duplicate-delivery/continuation tombstone window; parent receipts are retained only for live contexts;
