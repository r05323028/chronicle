...
7. Update validation groups, CI, docs, and `AGENTS.md`; run final coverage comparison (10.3) while all migration evidence is still retained, then migration bookkeeping cleanup (11.9), the no-dead-infrastructure invariant (11.10), and the final docs/`AGENTS.md` reconciliation (9.8) BEFORE collecting fresh/reused compatible gate evidence, so retained final evidence corresponds to the final documented source tree. Final coverage/runtime comparison MUST run while all migration evidence required to prove pre/post obligation mapping is still retained; task 11.9 may clean temporary bookkeeping only after task 10.3 completes.

Cutover vs physical deletion: migration tasks (e.g. 4.4, 8.5) perform cutover — create the replacement, prove it, switch callers/selectors/catalog ownership, prove zero active dependency on the legacy implementation, and mark the legacy item obsolete. Section 11 then performs physical deletion. Legacy implementations may physically exist between cutover and cleanup; physical deletion never occurs while active selectors/catalog/CI reference them. Task 9.7 completes final ACTIVE catalog/gate configuration before cleanup; task 11.9 may organize migration bookkeeping/history only AFTER final coverage/runtime comparison (10.3) completes and its evidence is retained, and must never destroy evidence required to reproduce or audit that comparison.
