...
Native execution lineage is additive non-v1 evidence. An exact ETL-bound `NativeExecutionLineage { parent, relation: ExecutionContinuation }` is candidate-specific, non-temporal, provider-neutral, and kept on the child correlation channel. Runtime receipts carry only opaque protocol-local boundary claims; they cannot construct or deserialize trusted protocol boundary identities. A registered protocol canonicalizer derives trusted boundary identity for ETL indexing. An application-local request counter or message ordinal is never binding authority. Parent and child receipts may complete in either order, but no partial anchor reaches ETL. Exact binding requires one canonical occurrence whose protocol identity, boundary direction, source generation, exact WAL range, and matching epoch-range placement all agree; response-side ranges cannot satisfy request boundaries. Binding ambiguity is a diagnostic, not resolver `Ambiguous`; only independently exact-bound positive parents can produce causal ambiguity. Cooperative source state retires completed child contexts, retains only a bounded tombstone window for duplicate delivery/continuation, and admits side-channel batches atomically; overflow retains no arbitrary positive subset and is retryable or typed loss.

The deterministic three-phase resolver lives beside this foundation in `chronicle-canonical`. Phase A1 propagates sparse `(operation, ScenarioId)` support sets by snapshot rounds to a fixed point without materializing outcomes; Phase A2 derives canonical ownership witnesses post-closure from immutable relation indexes plus final support sets, then materializes `Resolved`/`Ambiguous`/`Uncorrelated` exactly once; Phase B constructs direct causal-parent edges through a fixed normative sequence (invalid-relation pre-filter, unique-parent mapping, whole-graph cyclic-SCC analysis, internal-edge removal). Role evidence nested inside `InteractionRoleResolution` is classification provenance and never enters correlation indexes; only the explicit per-operation correlation-evidence channel feeds predicates (`SharedTraceIdentity`, directional child-to-parent `ExplicitParentSpan`). `ScenarioId`s derive as `scenario-id-v1`: SHA-256 over a fixed 36-byte domain separator plus the root's four UUID byte arrays in declaration order, truncated with RFC 9562 version-8/variant bits. Resolver-generated `ScenarioRoot` evidence is valid only in root-establishment placement and is rejected everywhere in caller input. Confidence reflects closed-support proofs (`Exact` direct/pinned, `Strong` purely transitive); temporal evidence is contextual only; retention targets 64 total retained correlation-evidence items per slot or ambiguity candidate, keeps required semantic witnesses in full, and uses remaining capacity for optional contextual fill via a total canonical evidence key; long proofs may exceed 64, and retention never feeds Stage B, which reads complete validated inputs. ETL composes through an explicit `CorrelationContext` join with zero selection semantics; the default publication/checkpoint path never invokes it.
