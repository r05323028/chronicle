...
Native execution lineage is additive non-v1 evidence. An exact ETL-bound `NativeExecutionLineage { parent, relation: ExecutionContinuation }` is candidate-specific, non-temporal, provider-neutral, and kept on the child correlation channel. Its boundary identity comes from the protocol canonicalizer's segmentation authority; an application-local request counter or message ordinal is never binding authority. Parent and child receipts may complete in either order, but no partial anchor reaches ETL. Binding ambiguity is a diagnostic, not resolver `Ambiguous`; only independently exact-bound positive parents can produce causal ambiguity.
