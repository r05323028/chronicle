...
The deterministic three-phase resolver lives beside this foundation in `chronicle-canonical`. Phase A1 propagates sparse `(operation, ScenarioId)` support sets by snapshot rounds to a fixed point without materializing outcomes; Phase A2 derives canonical ownership witnesses post-closure from immutable relation indexes plus final support sets, then materializes `Resolved`/`Ambiguous`/`Uncorrelated` exactly once; Phase B constructs direct causal-parent edges through a fixed normative sequence (invalid-relation pre-filter, unique-parent mapping, whole-graph cyclic-SCC analysis, internal-edge removal). Role evidence nested inside `InteractionRoleResolution` is classification provenance and never enters correlation indexes; only the explicit per-operation correlation-evidence channel feeds predicates (`SharedTraceIdentity`, `ExplicitParentSpan`). `ScenarioId`s derive as `scenario-id-v1`: SHA-256 over a fixed 36-byte domain separator plus the root's four UUID byte arrays in declaration order, truncated with RFC 9562 version-8/variant bits. Resolver-generated `ScenarioRoot` evidence is valid only in root-establishment placement and is rejected everywhere in caller input. Confidence reflects closed-support proofs (`Exact` direct/pinned, `Strong` purely transitive); temporal evidence is contextual only; retention (64 items per slot, applied per ambiguity candidate) happens strictly after semantics using a total canonical evidence key. ETL composes through an explicit `CorrelationContext` join with zero selection semantics; the default publication/checkpoint path never invokes it.
