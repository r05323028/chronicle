...
`InteractionRole` is application-relative: passive server ownership normalizes to `Ingress`, active outbound HTTP/database/cache ownership normalizes to `Egress`, and wire direction or socket role remains evidence only. `InteractionRoleResolution` preserves `Known`, `Unknown`, and candidate-specific `Ambiguous` outcomes. `CorrelationGraph` stores role state separately from `Resolved`, `Ambiguous`, and `Uncorrelated` correlation outcomes; unresolved operations remain discoverable, ambiguous candidates are not selected membership or edges, and only `Known(Ingress)` may be a scenario root. This foundation validates supplied outcomes; it does not infer ownership from raw interleaved traffic. A deterministic three-phase resolver in the canonical domain computes outcomes on demand from Chronicle-owned correlation evidence; ETL composes it through an explicit context join, never on the default publication path.
...
Native causal correlation has two explicit capability modes. **Passive-only Chronicle** remains safe and useful for capture, reconstruction, and canonicalization, but it does not infer application causality from timing, proximity, process/task/socket/connection identity, or wire direction. **Cooperative Chronicle-native mode** is opt-in: an application/runtime creates an opaque generation-safe Chronicle context, explicit child continuation, and protocol-owned pre-canonical boundary receipts. ETL binds both anchors exactly to canonical operations; zero or multiple matches fail closed. No tracing provider is required. Missing, ambiguous, lost, or over-capacity native observations remain unresolved and do not change completeness, replayability, WAL authority, or checkpoint semantics.
