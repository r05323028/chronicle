...
Correlation invariants: Chronicle owns application-relative role and scenario correlation; trace is optional opaque enrichment; role differs from byte direction; infrastructure identity is evidence, never scenario identity; ambiguity remains ambiguity; replayability/completeness stays separate. Correlation references use explicit recording/owner-epoch/session scope, and no `InteractionId` or `EventId` is added to frozen v1 contracts. The deterministic resolver keeps role evidence (classification provenance) separate from the correlation-evidence channel; scenario ownership materializes only after sparse support closure with witnesses derived post-closure; resolver-generated `ScenarioRoot` evidence is reserved from caller input and placement-validated by the domain; temporal evidence is contextual until a timeline-guarantee change exists; Stage B parent edges are constructed through the fixed pre-filter/unique-parent/SCC sequence, never by validator-rejection triage; native positive ownership predicates come only from exact ETL-bound `NativeExecutionLineage`; passive capture never supplies them.
